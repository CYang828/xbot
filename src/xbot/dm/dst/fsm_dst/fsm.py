...
class FSMDST(DST):
    ...
    def __init__(self):
        ...
        self.machine = Machine(model=self, states=self.restaurant_states, initial='greet')

        self.dest2trigger = {'推荐菜': {'trigger': 'inform_restaurant_dish', 'callback': 'set_recommend_dish'},
                             '评分': {'trigger': 'inform_restaurant_rating', 'callback': 'set_rating'},
                             '人均消费': {'trigger': 'inform_restaurant_avg_cost', 'callback': 'set_avg_cost'},
                             '周边酒店': {'trigger': 'inform_restaurant_surrounding_hotel',
                                      'callback': 'set_surrounding_hotel'},
                             '周边景点': {"trigger": 'inform_restaurant_surrounding_attraction',
                                      'callback': 'set_surrounding_attraction'},
                             '周边餐馆': {'trigger': 'inform_restaurant_surrounding_restaurant',
                                      'callback': 'set_surrounding_restaurant'}}

        for slot, trigger in self.dest2trigger.items():
            self.machine.add_transition(
                trigger=trigger['trigger'], source='*', dest=slot, before=trigger['callback']
            )
