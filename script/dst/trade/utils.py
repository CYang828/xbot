...
from data.crosswoz.data_process.dst.trade_preprocess import (
    EXPERIMENT_DOMAINS, Lang, get_seq, get_slot_information
)
...
def fix_general_label_error(belief_state):
    ...
    :return:
    """
    belief_state_dict = {slot_value['slots'][0][0]: slot_value['slots'][0][1]
                         for slot_value in belief_state}
    return belief_state_dict


def read_langs(file_name, gating_dict, slots, dataset, lang, mem_lang, load_lang, config):
...
def prepare_data_seq(config):
    eval_batch = config["eval_batch_size"] if config["eval_batch_size"] else config['batch_size']
    ...
    load_lang = False
    if (os.path.exists(lang_file_path) and os.path.exists(mem_lang_file_path)) and not config['clean_cache']:
    ...
    pair_train, train_max_len = read_langs(train_file_path, gating_dict, slots,
                                           "train", lang, mem_lang, load_lang, config)
    train_loader = get_seq(pair_train, lang, mem_lang, config['batch_size'],
                           config['n_gpus'], shuffle=True, config=config)
    train_vocab_size = lang.n_words

    pair_dev, dev_max_len = read_langs(dev_file_path, gating_dict, slots,
                                       "dev", lang, mem_lang, load_lang, config)
    dev_loader = get_seq(pair_dev, lang, mem_lang, eval_batch,
    ...
    # 保存中间数据
    if not (os.path.exists(lang_file_path) and os.path.exists(mem_lang_file_path)) or config['clean_cache']:
    ...
    if (not os.path.exists(emb_dump_path) or config['clean_cache']) and config["load_embedding"]:
        dump_pretrained_emb(config['orig_pretrained_embedding'], lang.index2word, emb_dump_path)
...
def masking(losses, mask):
    ...
    for si in range(mask.size(1)):
        ...
        seq_length_expand = mask[:, si].unsqueeze(1).expand_as(seq_range_expand)  # (bs, max_len)
