...
class Trainer:
    def __init__(self, config, langs, gating_dict, slots):
        ...
        self.model = Trade(lang=self.lang,
    ...
    def save_model(self, metric):
        ...
        torch.save(model_to_save.cpu().state_dict(), os.path.join(self.output_dir, name))
    ...
    def train_batch(self, data, slots, reset=0):
        ...
        loss_gate = self.cross_entropy(gates.contiguous().view(-1, gates.size(-1)),
        ...
        for j, data_dev in progress_bar:
            ...
            for bs in range(batch_size):
                ...
                for slot_id in range(num_slots):
                    one_words = [self.lang.index2word[word_id.item()] for word_id in words_id[bs][slot_id]]
                ...
                if self.use_gate:
                    for si, sg in enumerate(gate):  # (num_slots, )
                        if sg == self.gating_dict["none"]:
                            ...
                            predict_belief_bsz_ptr.append(slot_temp[si] + "-" + id2gate[sg.item()])
                else:
...
def main():
    ...
    lang = langs[0]
    model_config['pretrained_embedding_path'] = os.path.join(model_config['data_path'], f'emb{len(lang.index2word)}')

    print(f'>>> Train configs:')
    print('\t', model_config)

    # 初始化训练
    trainer = Trainer(config=model_config, langs=langs, gating_dict=gating_dict, slots=slots)
    ...
    for epoch in tqdm(range(start_epoch, model_config['num_epochs']), desc='Epoch'):
        ...
        if (epoch + 1) % int(model_config['eval_steps']) == 0:
            ...
            acc = trainer.evaluate(dev, avg_best, slots, epoch, model_config['early_stop'])
            ...
            if cnt == model_config["patience"] or (acc == 1.0 and model_config['early_stop'] is None):
