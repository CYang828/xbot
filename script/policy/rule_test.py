...
def eval_metrics(gold_pred_sys_das):
    ...
    for dia_id, sess in gold_pred_sys_das.items():
        for turn_id, turn in sess.items():
            ...
            elif ((turn['gold_sys_act'][0][0] == turn['pred_sys_act'][0][0] == 'Recommend')
    ...
    output_path = os.path.join(get_data_path(), 'crosswoz/policy_rule_single_domain_data/bad_case.json')
...
def main():
    ...
    train_path = os.path.join(get_data_path(), 'crosswoz/policy_rule_single_domain_data/train.json')
            ...
            if turn['role'] == 'usr':
                ...
                for domain, svs in turn['sys_state'].items():
                    for slot, value in svs.items():
                        if slot != 'selectedResults' and not rule_dst.state['belief_state'][domain][slot]:
