...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths):
        ...
        outputs = outputs[:, :, :self.hidden_size] + outputs[:, :, self.hidden_size:]  # 双向不拼接，相加
        return outputs.transpose(0, 1), hidden.unsqueeze(0)  # (bs, seq_len, hidden), (1, bs, hidden)
...
class Generator(nn.Module):
    ...
    def forward(self, batch_size, encoded_hidden, encoded_outputs, encoded_lens,
        ...
        all_point_outputs = torch.zeros(len(slot_temp), batch_size, max_res_len,
                                        self.vocab_size, device=story.device)
        all_gate_outputs = torch.zeros(len(slot_temp), batch_size,
                                       self.num_gates, device=story.device)

        # Get the slot embedding
        slot_emb_dict = {}
        # init
        slot_emb_arr = torch.zeros((len(slot_temp), batch_size, self.hidden_size),
                                   dtype=torch.float, device=story.device)
        for i, slot in enumerate(slot_temp):
            # init
            domain_emb = torch.zeros((1, self.hidden_size), dtype=torch.float, device=story.device)
            slot_emb = torch.zeros((1, self.hidden_size), dtype=torch.float, device=story.device)
        ...
        if self.parallel_decode:
            ...
            for wi in range(max_res_len):
                ...
                final_p_vocab = (1 - vocab_pointer_switches).expand_as(
                    p_context_ptr) * p_context_ptr + vocab_pointer_switches.expand_as(p_context_ptr) * p_vocab
            ...
            for slot in slot_temp:
                ...
                for wi in range(max_res_len):
                    ...
                    p_vocab = self.attend_vocab(self.embedding.weight, hidden.squeeze(0))  # (bs, vocab_size)
                    p_gen_vec = torch.cat([dec_state.squeeze(0), context_vec, decoder_input], -1)
                    vocab_pointer_switches = self.sigmoid(self.W_ratio(p_gen_vec))
                    p_context_ptr = torch.zeros_like(p_vocab, device=story.device)  # (bs, vocab_size)
                    p_context_ptr.scatter_add_(1, story, prob)  # prob: (bs, seq_len), story: (bs, seq)

                    final_p_vocab = (1 - vocab_pointer_switches).expand_as(
                        p_context_ptr) * p_context_ptr + vocab_pointer_switches.expand_as(p_context_ptr) * p_vocab
                    pred_word = torch.argmax(final_p_vocab, dim=1)  # (bs)

                    all_point_outputs[counter, :, wi, :] = final_p_vocab  # (num_slots, bs, max_res_len, vocab_size)
                    if use_teacher_forcing:
...
class Trade(nn.Module):
    ...
    def __init__(self, lang, vocab_size, hidden_size, dropout, num_encoder_layers, num_decoder_layers, pad_id, slots,
        ...
        self.encoder = EncoderRNN(vocab_size, hidden_size, dropout, n_layers=num_encoder_layers,
                                  pad_id=pad_id, pretrained_embedding_path=pretrained_embedding_path,
                                  load_embedding=load_embedding, fix_embedding=fix_embedding)
        self.decoder = Generator(lang, self.encoder.embedding, vocab_size, hidden_size, num_decoder_layers, dropout,
                                 slots, num_gates, parallel_decode)
    ...
    def forward(self, data, use_teacher_forcing, slots):
        ...
        all_point_outputs, all_gate_outputs = self.decoder(batch_size,
                                                           encoded_hidden,
                                                           encoded_outputs,
                                                           data['context_len'],
                                                           story, max_res_len,
...
class TradeDST(DST):
    ...
    def __init__(self):
        ...
        # load config
        common_config_path = os.path.join(get_config_path(), TradeDST.common_config_name)
        ...
        model = Trade(lang=self.lang, vocab_size=len(self.lang.index2word),
    ...
    def update(self, user_action):
        ...
        data_loader = prepare_data_for_update(self.model_config, self.lang, self.mem_lang, batch_size=1,
                                              source_text=source_text, curr_utterance=curr_utterance)

        with torch.no_grad():
            data = next(iter(data_loader))
            predict_belief = []
            # (1, num_slots, resp_len, vocab_size), (1, num_slots, num_gates)
            all_ptr_words_probs, gates_logits = self.model(data, use_teacher_forcing=False, slots=self.all_slots)
            gate_ids = gates_logits[0].argmax(dim=-1)  # (num_slots, )

            all_ptr_word_ids = all_ptr_words_probs[0].argmax(dim=-1)  # (num_slots, resp_len)
            num_slots, resp_len = all_ptr_word_ids.size()
            for i in range(num_slots):
                slot_value = 'none'

                if len(self.all_slots[i].split('-')) < 3 and self.id2gate[gate_ids[i].item()] == 'ptr':
                    resp_tokens = []
