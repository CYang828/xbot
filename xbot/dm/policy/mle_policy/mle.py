...
class MLEPolicy(Policy):
    ...
    def __init__(self):
        ...
        # load config
        common_config_path = os.path.join(get_config_path(), MLEPolicy.common_config_name)
        ...
        self.model_config['data_path'] = os.path.join(get_data_path(), 'crosswoz/policy_mle_data')
        ...
        for model_key, url in MLEPolicy.model_urls.items():
            ...
            file_name = model_key.split('.')[0] if not model_key.endswith('pth') else 'trained_model_path'
            self.model_config[file_name] = dst
            if not os.path.exists(dst) or not self.model_config['use_cache']:
                download_from_url(url, dst)

        self.vector = CrossWozVector(sys_da_voc_json=self.model_config['sys_da_voc'],
                                     usr_da_voc_json=self.model_config['usr_da_voc'])

        policy = MultiDiscretePolicy(self.vector.state_dim, model_config['hidden_size'],
    ...
    def predict(self, state):
        s_vec = torch.tensor(self.vector.state_vectorize(state), device=self.model_config['device'])
