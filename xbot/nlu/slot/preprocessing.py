    ...
    for key in data_key:
        ...
        for no, sess in data[key].items():
            ...
            for i, turn in enumerate(sess['messages']):
                ...
                processed_data[key].append([tokens,tags,golden,context[-context_size:]])
        ...
        # processed_data[key] = [[processed_data[key][i][0],processed_data[key][i][1],len(processed_data[key][i][-3])]for i in range(len(processed_data[key]))]
        json.dump(processed_data[key], open(os.path.join(processed_data_dir, '{}_data_zjw.json'.format(key)), 'w', encoding='utf-8'),
