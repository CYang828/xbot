...
class JointBERT(nn.Module):
    ...
    def forward(self, word_seq_tensor, word_mask_tensor, tag_seq_tensor=None, tag_mask_tensor=None,
        ...
        if not self.finetune:#如果不进行finetune，
            self.bert.eval()
            with torch.no_grad():#参数不更新
        ...
        sequence_output = outputs[0]#获取每个token的output 输出[batch_size, seq_length, embedding_size] 如果做seq2seq 或者ner 用这个
        pooled_output = outputs[1]#这个输出是获取句子的output

        if self.context and (context_seq_tensor is not None):#如果有上下文信息
            ...
            else:#将上下文信息进行bert训练并获得整个句子的output
                context_output = self.bert(input_ids=context_seq_tensor, attention_mask=context_mask_tensor)[1]
            sequence_output = torch.cat(
                [context_output.unsqueeze(1).repeat(1, sequence_output.size(1), 1),
                 sequence_output], dim=-1)#将上下文得到输出和word_seq_tensor得到的输出进行拼接
            pooled_output = torch.cat([context_output, pooled_output], dim=-1)#将上下文得到输出和之前获取句子的output进行拼接

        if self.hidden_units > 0:#经过dropout、Linear、relu层
        ...
        sequence_output = self.dropout(sequence_output)#经过dropout
        slot_logits = self.slot_classifier(sequence_output)#经过Linear层
        outputs = (slot_logits,)
