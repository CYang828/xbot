...
def get_single_domain_examples(input_file_path, output_file_path):
    ...
    single_domain_examples = {id_: dialogue for id_, dialogue in dataset.items()
                              if dialogue['type'] == '单领域'}

    print(f'{file_in_zip} total has {len(single_domain_examples)} single domain examples')

    dump_json(single_domain_examples, output_file_path)


def main():
    output_dir = os.path.join(get_data_path(), 'crosswoz/policy_rule_single_domain_data')
