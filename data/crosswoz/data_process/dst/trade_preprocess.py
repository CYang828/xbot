...
class Lang:
    def __init__(self, config):
        self.index2word = {config['pad_id']: "PAD", config['sos_id']: "SOS",
                           config['eos_id']: "EOS", config['unk_id']: 'UNK'}
...
class CNEmbedding:
    def __init__(self):
        vector_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                   'data', 'crosswoz', 'sgns.wiki.bigram.bz2')
        self.word2vec = {}
...
class CrossWOZDataset(Dataset):
    ...
    def preprocess(self, sequence, word2idx):
        ...
        story = [word2idx[word] if word in word2idx else self.config['unk_id']
    ...
    def preprocess_slot(self, sequence, word2idx):
        ...
        for value in sequence:
            ...
            v = [word2idx[word] if word in word2idx else self.config['unk_id']
                 for word in value.split()] + [self.config['eos_id']]
            story.append(v)
        # story = torch.Tensor(story)
        return story
...
def collate_fn(data, pad_id=1, n_gpus=0):
    ...
    y_seqs, y_lengths = merge_multi_response(item_info["generate_y"])  # (bs, num_slots, max_len)
...
class ImbalancedDatasetSampler(Sampler):
    ...
    def __init__(self, dataset, indices=None, num_samples=None):
        ...
        weights = [1.0 / sum([label_to_count[label] for label in self._get_label(dataset, idx)]) for idx in
                   self.indices]
    ...
    def __iter__(self):
        return (self.indices[i.item()] for i in torch.multinomial(self.weights, self.num_samples, replacement=True))
...
def get_seq(pairs, lang, mem_lang, batch_size, n_gpus, shuffle, config):
    ...
    dataset = CrossWOZDataset(data_info, lang.word2index, lang.word2index, mem_lang.word2index, config)
...
def prepare_data_for_update(config, lang, mem_lang, batch_size=100,
                            source_text='', curr_utterance=''):

    utterance = curr_utterance
    if source_text == '':
        source_text = "; " + utterance.strip() + " ;"
    pair_test = read_langs_for_update(source_text, utterance)
    data_loader = get_seq(pair_test, lang, mem_lang, batch_size,
